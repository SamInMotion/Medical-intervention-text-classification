...
## Figure 1 (`outputs/fig1_gap_forest.pdf`) is generated by
`scripts/make_fig1_gap_forest.py`, which reads the multi-run BoW and multi-seed
BERT summary JSONs and writes directly to this filename. `scripts/make_paper_artifacts.py`
also names this file but does not run on the current tree; it expects per-fold
files without the seed suffix the multi-seed protocol introduced (`PROVENANCE.md`
#43, unrelated to Figure 1's actual generator). The archived per-fold outputs
under `outputs/` reproduce every reported value.
...
`scripts/make_fig1_gap_forest.py` is the working generator for
`outputs/fig1_gap_forest.pdf` (see the Figure 1 section above).
`scripts/make_paper_artifacts.py` also names this file but does not run on the
current tree; see `PROVENANCE.md` #43.
