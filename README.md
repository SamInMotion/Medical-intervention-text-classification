...
5-fold stratified cross-validation across all 11 configurations gives 80–86% accuracy on 150 manually annotated abstracts. The simplest configurations (raw trigrams, no ontology enrichment) perform as well as or better than enriched ones; a finding that revised the original thesis results.
...
5-fold stratified cross-validation with ROC AUC: python -m src.pipeline --kfold 5 --all-workflows
## Setup
...
-Benchmark dataset integration (Cohen et al. 2006, CLEF eHealth TAR 2017-2019) to test whether ontology enrichment helps on larger, more diverse corpora
-BioBERT/PubMedBERT fine-tuning to compare transformer approaches against the bag-of-words baseline
