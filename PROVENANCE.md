...
| 42 | internal context-update documents and consolidation drafts tracked in the repo; identity-bearing | OPEN |
| 43 | `bootstrap_paired_permutation.py` and `scripts/make_paper_artifacts.py` both expect `bert_{topic}_{mode}.txt` without the seed suffix the multi-seed protocol introduced. Neither runs on the current tree; `make_paper_artifacts.py` fails with ZeroDivisionError on an empty array. | OPEN |
...
| 53b | **The figure namespace is inverted, and that is how #53 stayed hidden.** `scripts/make_paper_artifacts.py` → `fig1_gap_forest.pdf` (**consumed**); `scripts/make_fig1_gap_forest.py` writes fig1_gap_forest_v3.pdf (not consumed); `make_fig1_v2.py` → `_v2` (not consumed, stale); `make_fig2_design_sensitivity.py` → `fig2_design_sensitivity.pdf` (not consumed, stale). The file named after the shipped figure does not produce it; the file that does is named after something else; the highest version number belongs to a file nothing consumes. ... | Header added to `make_fig1_gap_forest.py` 08-11; rename still OPEN. `make_paper_artifacts.py` names `fig1_gap_forest.pdf` but cannot produce it on the current tree (#43), so Figure 1's generator is identified but not executable — `git grep savefig` proved the filename, not that it runs |
